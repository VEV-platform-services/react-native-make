...
const addLaunchScreenBackgroundColor = (backgroundColor) => {
    file_processing_1.replaceInFile(path_1.join(__dirname, '../../../../templates/android/values/colors-splash.xml'), `${utils_1.getAndroidResourceOutputPath()}/values/colors-splash.xml`, [
        {
...
const generateAndroidSplashImages = (imageSource) => Promise.all(config_2.config.androidSplashImages.map(({ size, density }) => image_processing_1.generateResizedAssets(imageSource, `${utils_1.getAndroidResourceOutputPath()}/drawable-${density}/splash_image.png`, size, size, {
    fit: 'inside',
})));
